    ...
    <div class="flex items-center space-x-8">
      ...
      <div class="flex gap-4">
        @if (Auth::check() && Auth::user()->role === 'admin')
          <a href="{{ route('auth.show') }}" class="relative px-4 py-2 rounded-lg text-white font-semibold hover:bg-cyan-700/70 hover:text-yellow-300 transition-all duration-300 shadow hover:scale-105">Daftar Pengguna</a>
        @else
        @endif
        <!-- User Dropdown -->
        <div x-data="{ open: false }" class="relative">
          ...
          <div x-show="open" x-transition:enter="transition ease-out duration-300"
            ...
            class="absolute right-0 mt-3 w-52 bg-white/90 backdrop-blur-md rounded-xl shadow-2xl ring-1 ring-cyan-200 z-50 origin-top-right"
            style="display: none;">
            @if (Auth::check())
